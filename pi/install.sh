...
install -m 644 node.py buzzer.py avatar.py eye.py autoportrait.py twilight.py index.html /opt/testate/
...
install -m 644 testate.service testate-heartbeat.service testate-heartbeat.timer \
    testate-observe.service testate-observe.timer \
    testate-twilight.service testate-twilight.timer /etc/systemd/system/
systemctl daemon-reload
systemctl enable --now avahi-daemon testate.service testate-heartbeat.timer \
    testate-observe.timer testate-twilight.timer
